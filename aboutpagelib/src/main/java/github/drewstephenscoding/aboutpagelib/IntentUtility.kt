...
private val REDDIT_URL_PREFIX = "https://www.reddit.com"
private val REDDIT_PACKAGE_ID = "com.reddit.frontpage"
private val LINKEDIN_PACKAGE_ID = "com.linkedin.android"
private val LINKEDIN_URL_PREFIX = "https://www.linkedin.com/in/"
...
internal fun openReddit(context: Context, userId: String){
    val redditIntent = Intent(Intent.ACTION_VIEW, Uri.parse(REDDIT_URL_PREFIX + userId))
    try {
        redditIntent.setPackage(REDDIT_PACKAGE_ID)
        context.startActivity(redditIntent)
    }catch (e: ActivityNotFoundException) {
        Log.e(TAG, "Reddit application uninstalled on current device")
        context.startActivity(redditIntent)
    }
}

internal fun openLinkedIn(context: Context, userId: String){
    val linkedInIntent = Intent(Intent.ACTION_VIEW, Uri.parse(LINKEDIN_URL_PREFIX + userId))
    try {
        linkedInIntent.setPackage(LINKEDIN_PACKAGE_ID)
        context.startActivity(linkedInIntent)
    }catch (e: ActivityNotFoundException) {
        Log.e(TAG, "LinkedIn application uninstalled on current device")
        context.startActivity(linkedInIntent)
    }
}